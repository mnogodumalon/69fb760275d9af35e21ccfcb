...
export default function BestellungenPage() {
  ...
  const [laptop_katalogList, setLaptopKatalogList] = useState<LaptopKatalog[]>([]);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, laptop_katalogData] = await Promise.all([
        LivingAppsService.getBestellungen(),
        LivingAppsService.getLaptopKatalog(),
      ]);
      setRecords(mainData);
      setLaptopKatalogList(laptop_katalogData);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    ...
    >
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('adresse_strasse')}>
                <span className="inline-flex items-center gap-1">
                  Straße
                  {sortKey === 'adresse_strasse' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('adresse_hausnummer')}>
                <span className="inline-flex items-center gap-1">
                  Hausnummer
                  {sortKey === 'adresse_hausnummer' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('adresse_plz')}>
                <span className="inline-flex items-center gap-1">
                  PLZ
                  {sortKey === 'adresse_plz' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('adresse_ort')}>
                <span className="inline-flex items-center gap-1">
                  Ort
                  {sortKey === 'adresse_ort' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('laptop_ref')}>
                <span className="inline-flex items-center gap-1">
                  Laptop
                  {sortKey === 'laptop_ref' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('bestelldatum')}>
                <span className="inline-flex items-center gap-1">
                  Bestelldatum
                  {sortKey === 'bestelldatum' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('bestellstatus')}>
                <span className="inline-flex items-center gap-1">
                  Bestellstatus
                  {sortKey === 'bestellstatus' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell>{record.fields.adresse_strasse ?? '—'}</TableCell>
                <TableCell>{record.fields.adresse_hausnummer ?? '—'}</TableCell>
                <TableCell>{record.fields.adresse_plz ?? '—'}</TableCell>
                <TableCell>{record.fields.adresse_ort ?? '—'}</TableCell>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getLaptopKatalogDisplayName(record.fields.laptop_ref)}</span></TableCell>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{record.fields.bestellstatus?.label ?? '—'}</span></TableCell>
      ...
      <BestellungenDialog
        ...
        laptop_katalogList={laptop_katalogList}
