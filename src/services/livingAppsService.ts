...
import type { LaptopKatalog, Bestellungen, CreateLaptopKatalog, CreateBestellungen } from '@/types/app';
...
export class LivingAppsService {
  // --- LAPTOP_KATALOG ---
  static async getLaptopKatalog(): Promise<LaptopKatalog[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.LAPTOP_KATALOG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as LaptopKatalog[];
    return enrichLookupFields(records, 'laptop_katalog');
  }
  static async getLaptopKatalogEntry(id: string): Promise<LaptopKatalog | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.LAPTOP_KATALOG}/records/${id}`);
    const record = { record_id: data.id, ...data } as LaptopKatalog;
    return enrichLookupFields([record], 'laptop_katalog')[0];
  }
  static async createLaptopKatalogEntry(fields: CreateLaptopKatalog) {
    return callApi('POST', `/apps/${APP_IDS.LAPTOP_KATALOG}/records`, { fields: cleanFieldsForApi(fields as any, 'laptop_katalog') });
  }
  static async updateLaptopKatalogEntry(id: string, fields: Partial<CreateLaptopKatalog>) {
    return callApi('PATCH', `/apps/${APP_IDS.LAPTOP_KATALOG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'laptop_katalog') });
  }
  static async deleteLaptopKatalogEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.LAPTOP_KATALOG}/records/${id}`);
  }
  ...
  static async getBestellungen(): Promise<Bestellungen[]> {
    ...
    return enrichLookupFields(records, 'bestellungen');
  }
  static async getBestellungenEntry(id: string): Promise<Bestellungen | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Bestellungen;
    return enrichLookupFields([record], 'bestellungen')[0];
  }
  static async createBestellungenEntry(fields: CreateBestellungen) {
    return callApi('POST', `/apps/${APP_IDS.BESTELLUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'bestellungen') });
  }
  static async updateBestellungenEntry(id: string, fields: Partial<CreateBestellungen>) {
    return callApi('PATCH', `/apps/${APP_IDS.BESTELLUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'bestellungen') });
  }
