...
export function BestellungenDialog({ open, onClose, onSubmit, defaultValues, laptop_katalogList, enablePhotoScan = true, enablePhotoLocation = true }: BestellungenDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="laptop_ref" entity="Laptop-Katalog">\n${JSON.stringify(laptop_katalogList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "kunde_vorname": string | null, // Vorname\n  "kunde_nachname": string | null, // Nachname\n  "kunde_email": string | null, // E-Mail\n  "kunde_telefon": string | null, // Telefonnummer\n  "adresse_strasse": string | null, // Straße\n  "adresse_hausnummer": string | null, // Hausnummer\n  "adresse_plz": string | null, // PLZ\n  "adresse_ort": string | null, // Ort\n  "laptop_ref": string | null, // Display name from Laptop-Katalog (see <available-records>)\n  "menge": number | null, // Menge\n  "bestelldatum": string | null, // YYYY-MM-DD\n  "zahlungsart": LookupValue | null, // Zahlungsart (select one key: "vorkasse" | "rechnung" | "paypal" | "kreditkarte") mapping: vorkasse=Vorkasse, rechnung=Rechnung, paypal=PayPal, kreditkarte=Kreditkarte\n  "lieferadresse_abweichend": boolean | null, // Abweichende Lieferadresse\n  "bestellstatus": LookupValue | null, // Bestellstatus (select one key: "offen" | "in_bearbeitung" | "versandt" | "abgeschlossen" | "storniert") mapping: offen=Offen, in_bearbeitung=In Bearbeitung, versandt=Versandt, abgeschlossen=Abgeschlossen, storniert=Storniert\n  "bemerkungen": string | null, // Bemerkungen\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["laptop_ref"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = v;
        }
        const laptop_refName = raw['laptop_ref'] as string | null;
        if (laptop_refName) {
          const laptop_refMatch = laptop_katalogList.find(r => matchName(laptop_refName!, [String(r.fields.hersteller_name ?? '')]));
          if (laptop_refMatch) merged['laptop_ref'] = createRecordUrl(APP_IDS.LAPTOP_KATALOG, laptop_refMatch.record_id);
        }
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="laptop_ref">Laptop</Label>
            <Select
              value={extractRecordId(fields.laptop_ref) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, laptop_ref: v === 'none' ? undefined : createRecordUrl(APP_IDS.LAPTOP_KATALOG, v) }))}
            >
              <SelectTrigger id="laptop_ref"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                {laptop_katalogList.map(r => (
                  <SelectItem key={r.record_id} value={r.record_id}>
                    {r.fields.hersteller_name ?? r.record_id}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
